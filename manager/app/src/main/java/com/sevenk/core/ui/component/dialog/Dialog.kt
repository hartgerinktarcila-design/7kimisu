...
/**
 * 「只读告知」弹窗的句柄 —— 跨两套界面通用，只有一个「知道了」按钮。
 *
 * 用途：需要把一件事**讲清楚**、但不需要用户做选择的地方
 * （例如首页那个「内核还没认主」——以前点了没反应，用户以为界面坏了）。
 */
class InfoDialogHandle internal constructor(
    private val visible: MutableState<Boolean>,
    private val titleState: MutableState<String>,
    private val contentState: MutableState<String>,
) {
    val isShown: Boolean
        get() = visible.value
...
/**
 * 建一个 [InfoDialogHandle] 并把弹窗挂到当前组合里。
 *
 * 与 `rememberConfirmDialog` 的区别：那个**一定画两个按钮**
 * （`dismiss` 传 null 时也退化成「取消」），做纯告知会让用户不知道该点哪个。
 */
@Composable
fun rememberInfoDialog(): InfoDialogHandle {
    val visible = rememberSaveable { mutableStateOf(false) }
    val title = rememberSaveable { mutableStateOf("") }
    val content = rememberSaveable { mutableStateOf("") }
    val onDismiss = { visible.value = false }

    when (LocalUiMode.current) {
        UiMode.Miuix -> InfoDialogMiuix(visible.value, title.value, content.value, onDismiss)
        UiMode.Material -> InfoDialogMaterial(visible.value, title.value, content.value, onDismiss)
    }

    return remember { InfoDialogHandle(visible, title, content) }
}
