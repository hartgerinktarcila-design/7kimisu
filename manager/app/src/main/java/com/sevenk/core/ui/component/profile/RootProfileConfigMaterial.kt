...
import com.sevenk.core.profile.Capabilities
import com.sevenk.core.profile.Groups
import com.sevenk.core.toRawFlags
import com.sevenk.core.toRootProfileFlags
...
import com.sevenk.core.ui.component.profile.dialogs.MultiSelectDialog
import com.sevenk.core.ui.component.profile.dialogs.SingleSelectDialog
import com.sevenk.core.ui.util.isSepolicyValid
...
private fun SELinuxDialog(
    ...
    // 🔴（v2.18）同 RootProfileConfigMiuix：`isSepolicyValid()` 是 root shell 往返，
    // 不能在组合期（主线程）跑，也不能每改一个字符就同步跑一次。
    var isRulesValid by remember { mutableStateOf(true) }
    LaunchedEffect(currentRules) {
        isRulesValid = withContext(Dispatchers.IO) { isSepolicyValid(currentRules) }
    }
