...
import com.sevenk.core.ui.component.miuix.SuperEditArrow
...
private fun SELinuxPanel(
    ...
    // 🔴（v2.18）`isSepolicyValid()` 内部要跑 `ksud sepolicy check`（root shell 往返）。
    // 旧写法 `remember(rules) { isSepolicyValid(rules) }` 在**组合期（主线程）**同步执行，
    // 而且 rules 每改一个字符就重跑一次 ⇒ 打开这个面板就卡、编辑时更容易 ANR。
    // 改成挂起侧（IO）计算，结果回来再刷新错误态。
    var isRulesValid by remember { mutableStateOf(true) }
    LaunchedEffect(rules) {
        isRulesValid = withContext(Dispatchers.IO) { isSepolicyValid(rules) }
    }
