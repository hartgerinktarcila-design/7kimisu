...
class DownloadService : Service() {
    ...
    companion object {
        ...
        const val ACTION_DOWNLOAD = "com.sevenk.core.action.DOWNLOAD"
        const val ACTION_CANCEL = "com.sevenk.core.action.CANCEL_DOWNLOAD"
        const val ACTION_DISMISS_DOWNLOAD = "com.sevenk.core.action.DISMISS_DOWNLOAD"
        const val ACTION_INSTALL_MODULE = "com.sevenk.core.action.INSTALL_MODULE"
    ...
    private fun startDownload(id: Int, url: String, fileName: String) {
        val job = serviceScope.launch {
            ...
            // 🟠（v2.18）"这一份文件到底成没成"的标记，给 finally 决定要不要删半截文件
            var completed = false
            try {
                ...
                // 🟠（v2.18）**没成功就删掉半截文件**（取消 / 断网 / 磁盘满 / HTTP 错误都算）。
                // 旧写法两条失败路径都不删 ⇒ Downloads 里留下半截 `xxx.zip`，
                // 下次下载被系统另存成 `xxx (1).zip`；用户手刷残包 = 装坏模块。
                if (!completed) runCatching { if (target.isFile) target.delete() }
