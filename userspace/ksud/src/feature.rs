...
pub fn save_binary_config(features: &HashMap<u32, u64>) -> Result<()> {
    ...
    let path = Path::new(FEATURE_CONFIG_PATH);
    // 🟠6（v2.4）／🟡（v2.19）：**原子写**。旧写法 `File::create(path)` 直接截断目标文件：
    // 写到一半掉电 → magic 校验失败 → **所有 feature 回落默认**
    // （su 兼容 / kernel umount / sulog / adb root / selinux hide 一次性全变）。
    //
    // v2.4 起这里自己抄了一份 `temp + sync_all + rename`（照 `module_config.rs:214-267`
    // 的上游写法）。v2.19 统一改成调用 `atomic::write_atomic`：
    //   · 临时名从固定的 `features.tmp`（`with_extension("tmp")`）换成
    //     `.<原名>.tmp-<pid>-<uniq>` —— 固定名在**跨进程**并发写时会互相截断，
    //     结果可能 rename 出一份半截配置；
    //   · 失败路径（写失败 / rename 失败）**一定会清掉临时文件**，旧写法会留垃圾。
    // 对外语义不变：要么还是完整的旧文件，要么已经是完整的新文件。
    let mut buf = Vec::with_capacity(16 + features.len() * 16);
    ...
    crate::atomic::write_atomic(path, &buf, None)
...
pub fn check_feature(id: &str) -> Result<()> {
    ...
    //
    // ⚠️ v2.14：**拿不到内核句柄时必须优雅降级**，不能把错误一路抛到 main。
    //    这条子命令是管理器 App 的固定探测（`libksud.so feature check <name>`，
    //    见 `KsuCli.getFeatureStatus`）。在部分 ROM（vivo/OriginOS、OPPO、MIUI…）上，
    //    内核访问会因为 seccomp 而被拦掉（机制见 ksucalls.rs 顶部那段长注释）。
    //    旧写法是 `.with_context(...)?` —— 内核访问一不可用，进程就带着非 0 退出码
    //    和一条英文错误结束，App 只能看到"命令失败"。
    //    现在统一输出 `unsupported`（= 不支持 / 未知）并 **exit 0**，
    //    与 `list_features` 的 `NOT_SUPPORTED` 语义保持一致。
    let supported = match crate::ksucalls::get_feature(feature_id as u32) {
        Ok((_value, supported)) => supported,
        Err(e) => {
            log::warn!("feature check {id}: no kernel access ({e:#}); reporting unsupported");
            false
        }
    };
