...
# 🟡7（v2.4）：`rm -rf` 之前的**路径护栏**。
#
# 背景：`MODID` 来自模块 zip 里的 `module.prop`；一旦为空或变成 `..`，
# `MODPATH=$MODULEROOT/$MODID` 就退化成 **模块根目录**（`/data/adb/modules_update/`），
# 于是 `rm -rf $MODPATH` 会**清空整个模块根**（连面具/别人的模块一起）。
# 所以任何删 $MODPATH 的地方都必须先过这里：只允许删"模块根下面的**一层具体目录**"。
safe_rm_modpath() {
  [ -n "$MODID" ] || return 1
  [ -n "$MODPATH" ] || return 1
  # 最后一个路径组件必须是个正常名字（空 / . / .. 一律拒）
  case "${MODPATH##*/}" in
    ""|"."|"..") return 1 ;;
  esac
  # 不许出现 `..` 路径组件
  ...
  # 必须是 /data/adb/modules/<id> 或 /data/adb/modules_update/<id>
  case "$MODPATH" in
...
abort() {
  ...
  # 🟡7（v2.4）：走护栏；路径不正常就**不删**（宁可留下垃圾，也不能清空模块根）
  safe_rm_modpath || true
  rm -rf $TMPDIR
  exit 1
}
...
install_module() {
  ...
  # 🟡7（v2.4）：没有合法 id 就没有合法的 MODPATH —— 之后所有 rm -rf 都会失去目标。
  [ -n "$MODID" ] || abort "! module.prop has no valid 'id'"
  case "$MODID" in
  ...
  # Create mod paths
  # 🟡7（v2.4）：删之前过护栏 —— MODID 为空/异常时这里会 abort，而不是清空模块根
  safe_rm_modpath || abort "! Refusing to wipe an unexpected module path: $MODPATH"
  mkdir -p $MODPATH
