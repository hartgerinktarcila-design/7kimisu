...
pub fn run(package_name: &String, kmi: Option<String>, allow_shell: bool) -> Result<()> {
    ...
    if ksuinit::has_kernelsu() {
        ...
        // 3. Get sevenk.ko from embedded assets
        let ko_name = format!("{kmi}_sevenk.ko");
        let ko_data = assets::get_asset_data(&ko_name)
            .with_context(|| format!("Failed to get {ko_name} from assets"))?;

        // 4. Load sevenk.ko from memory with manual relocation
        info!("Loading sevenk.ko for KMI {kmi}...");
        ...
        ksuinit::load_module(&ko_data, params).context("Failed to load sevenk.ko")?;
        info!("sevenk.ko loaded successfully!");
    ...
    let _ = Command::new("am")
        .args([
            "start",
            "-n",
            &format!("{package_name}/com.sevenk.core.ui.MainActivity"),
        ])
        .status();
