...
pub struct BootPatchV2Args {
    ...
    /// Exact sevenk.ko; auto-select the embedded KMI build when omitted
    #[arg(short, long)]
...
fn embedded_module_name(kmi: &str) -> String {
    ...
    {
        format!("{kmi}_sevenk.ko")
    }
    #[cfg(not(target_os = "android"))]
    {
        format!("aarch64/{kmi}_sevenk.ko")
    }
}
...
pub fn patch_boot(args: &BootPatchV2Args) -> Result<()> {
    ...
    // 🟡（v2.19）旧写法是 `temporary.flush()` —— 而 `flush()` 对 `std::fs::File` 是
    // **空操作**（只有带缓冲的 writer 才需要它）。紧接着的 `persist()` 走的是 `rename`：
    // 掉电时完全可能"改名已生效、数据还没进闪存" ⇒ 落出来的 boot 镜像是**空的**
    // ⇒ 刷进去直接引导失败。改成 `sync_all()`（与 v2.18 给 `copy_verify` 补 sync 同一个道理）。
    temporary
        .as_file()
        .sync_all()
        .context("cannot sync patched boot image")?;
    #[cfg(unix)]
    {
...
mod tests {
    ...
    fn embedded_module_uses_release_asset_layout() {
        ...
        #[cfg(target_os = "android")]
        assert_eq!(name, "android12-5.10_sevenk.ko");
        #[cfg(not(target_os = "android"))]
        assert_eq!(name, "aarch64/android12-5.10_sevenk.ko");
