...
pub fn ensure_symlink(module_path: &Path) -> Result<()> {
    ...
    if symlink_path.is_symlink() {
        info!("Removing old metamodule symlink");
        std::fs::remove_file(symlink_path).with_context(|| "Failed to remove old symlink")?;
    } else if symlink_path.exists() {
        // 🟡8（v2.4）：`/data/adb/metamodule` **本该是一个软链接**（由本函数创建）。
        // 旧写法见到"不是软链接"就 `remove_dir_all` —— 万一那里是个真目录（用户/别的工具
        // 放的东西），这一下就是不可逆的误删。现在**拒绝**，让人自己看清楚再处理。
        bail!(
            "{} exists but is NOT a symlink; refusing to delete it.\n\
             Please inspect/remove it manually, then install the metamodule again.",
            symlink_path.display()
        );
    }
...
pub fn exec_metauninstall_script(module_id: &str) -> Result<()> {
    ...
    // 🟠（v2.19）旧写法是 `metauninstall_path.to_str().unwrap()` +
    // `.current_dir(metauninstall_path.parent().unwrap())`（上游同位置同款）。
    // 路径含非 UTF-8 字节、或没有父目录时 ksud 会**直接 panic** ——
    // 而 release 是 `panic = "abort"`（SIGABRT），且这是 `on_post_data_fs` 的**开机链路**。
    // 改成优雅失败：报上下文错误返回，进程不死。
    let Some(metauninstall_script) = metauninstall_path.to_str() else {
        anyhow::bail!(
            "metamodule metauninstall.sh path is not valid UTF-8: {}",
            metauninstall_path.display()
        );
    };
    let Some(metauninstall_dir) = metauninstall_path.parent() else {
        anyhow::bail!(
            "metamodule metauninstall.sh path has no parent directory: {}",
            metauninstall_path.display()
        );
    };
    let result = Command::new(assets::BUSYBOX_PATH)
        .args(["sh", metauninstall_script])
        .current_dir(metauninstall_dir)
...
pub fn exec_mount_script(module_dir: &str) -> Result<()> {
    ...
    // 🟠（v2.19）同 `exec_uninstall_script`：旧写法 `.to_str().unwrap()` 在非 UTF-8
    // 路径上直接 panic（开机链路、`panic = "abort"`）⇒ 改成优雅失败。
    let Some(mount_script_str) = mount_script.to_str() else {
        anyhow::bail!(
            "metamodule mount script path is not valid UTF-8: {}",
            mount_script.display()
        );
    };
    let result = Command::new(assets::BUSYBOX_PATH)
        .args(["sh", mount_script_str])
        .envs(crate::module::get_common_script_envs(
            get_metamodule_id().as_deref(),
        ))
