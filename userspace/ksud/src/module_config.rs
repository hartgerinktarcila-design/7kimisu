...
pub fn load_config(module_id: &str, config_type: ConfigType) -> Result<HashMap<String, String>> {
    ...
    // 🟡8（v2.4）：**读之前先上界**。这是个可以被外部改写的二进制文件，
    // 畸形 `count` / `key_len` / `value_len` 会让下面 `vec![0u8; len]`
    // 直接申请几个 GB（OOM），或者白转 40 亿圈。上界与写路径的校验常量完全一致。
    if count as usize > MAX_CONFIG_COUNT {
        bail!("Config has too many entries: {count} (max: {MAX_CONFIG_COUNT})");
    }
    ...
    for i in 0..count {
        ...
        if key_len > MAX_CONFIG_KEY_LEN {
            bail!("Config key length too large for entry {i}: {key_len} (max: {MAX_CONFIG_KEY_LEN})");
        }
        ...
        if value_len > MAX_CONFIG_VALUE_LEN {
            bail!(
                "Config value length too large for entry {i}: {value_len} (max: {MAX_CONFIG_VALUE_LEN})"
            );
        }
