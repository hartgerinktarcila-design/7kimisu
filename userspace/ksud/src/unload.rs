...
pub fn unload() -> Result<()> {
    ...
    // 🟠（v2.18）**失败不能当成功**：旧写法只 `warn!` 一句，然后照样 `exit(0)` ——
    // 调用方（App / 脚本）看到退出码 0 就报告"已卸载"，而内核模块其实还在。
    // 现在把结论带到退出码上（服务照旧重启，不因为这一步失败就不管用户）。
    // `ENOENT`（模块本来就不在）与"卸载成功"等价，算成功。
    let removed = match rustix::system::delete_module(c"kernelsu", 0) {
        // `ENOENT`（模块本来就不在）与"卸载成功"等价，算成功
    ...
    // 6. Exit
    if removed {
        info!("unload: done (kernelsu module removed), exiting ksud");
        std::process::exit(0);
    }
    warn!("unload: 内核模块没能卸载（见上面的 delete_module 错误）⇒ 以非 0 退出码如实上报");
    std::process::exit(1);
}
