...
/// 2026-09-21 安全护栏（B6）：取 vermagic 里的**内核 KMI**（主.次，例如 `6.12`）。
///
/// 规则（和 ksud 侧的检查是同一套）：
///   · 模块 vermagic 与内核要求的 vermagic，**主.次必须完全一致**；
///   · 允许不同的只有 **sublevel（第三段）** 和尾部组件
///     （SMP / preempt / mod_unload / modversions / 架构标签 / 工具链构建后缀）。
///
/// ⚠️ 为什么不是"主.次.修全等"（2026-09-21 实测修正，很重要）：
///   我们发给用户的 `.ko` 是**按 KMI 预编译**的通用模块（文件名 `android16-6.12_sevenk.ko`），
///   它的构建版本号（实测 **6.12.76**）与用户设备内核的 patchlevel
///   （本项目那台平板是 **6.12.30**）**本来就不会相等**。
///   要求 `x.y.z` 全等 = 正常用户**永远加载不了模块**（"装了没 root"），是功能回归；
///   而"主.次相同"正是 Android KMI 的兼容口径，也正是任务书给的兜底口径
///   （「至少限制到仅 sublevel 差异」）。
///
/// 为什么这样仍能防住硬砖：主.次不同 = 不是同一代内核，结构体布局/符号很可能对不上，
/// 改写 vermagic 再 force-load 会直接内核 panic。这条路径现在**在重试之前就返回错误**
/// （下面的 `load_module` 只把错误写进 klog，然后照常 exec 真正的 init → 设备仍能开机）。
fn kernel_version_prefix(vermagic: &str) -> Option<String> {
    // vermagic 形如 `5.15.149-android13-8-g1234 SMP preempt mod_unload modversions aarch64`
    let token = vermagic.split_whitespace().next()?;
...
/// 从 ELF 的 `.modinfo` 段里读出模块自己的 `vermagic=...` 条目。
fn read_module_vermagic(buffer: &[u8]) -> Option<String> {
    ...
    for section in elf.section_headers.iter() {
        ...
        let offset = usize::try_from(section.sh_offset).ok()?;
        let size = usize::try_from(section.sh_size).ok()?;
        let data = buffer.get(offset..offset.checked_add(size)?)?;

        for entry in data.split(|byte| *byte == 0) {
            if let Some(value) = entry.strip_prefix(b"vermagic=") {
...
pub fn load_module(data: &[u8], params: &CStr) -> Result<()> {
    ...
    match init_module(&buffer, params) {
        ...
        Err(first_error) => {
            ...
            // 2026-09-21 安全护栏（B6）：只在**内核 KMI（主.次）一致**时才允许改写 vermagic 重试。
            // 见 kernel_version_prefix 的注释：KMI 不同 = 不是同一代内核，force-load 会内核 panic；
            // 而 sublevel（第三段）不同是常态（我们的 .ko 是按 KMI 预编译的），必须放行。
            let module_vermagic = read_module_vermagic(&buffer);
            let module_prefix = module_vermagic.as_deref().and_then(kernel_version_prefix);
            let required_prefix = kernel_version_prefix(&required_vermagic);

            if module_prefix.is_none() || module_prefix != required_prefix {
                log::error!(
                    "module/kernel mismatch: module vermagic {:?} (KMI {:?}) vs kernel required {:?} (KMI {:?}); refusing to force-load",
                    module_vermagic,
                    module_prefix,
                    required_vermagic,
                    required_prefix
                ...
                bail!(
                    "module/kernel mismatch: module is built for the Linux {:?} KMI but the running kernel is the {:?} KMI; \
                     refusing to rewrite vermagic and force-load (that could panic the kernel)",
                    module_prefix.as_deref().unwrap_or("<unknown>"),
                    required_prefix.as_deref().unwrap_or("<unknown>")
                );
            }

            log::warn!(
                "Kernel requires vermagic {:?}; module KMI {:?} matches, replacing only the sublevel/trailing components and retrying",
                required_vermagic,
                module_prefix
            );
